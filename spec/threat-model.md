...
- Tool + spec mirrored beyond our control: GitHub (primary), Codeberg, GitLab,
...
## T9. Cryptographic obsolescence (including quantum)

**The attack.** A future adversary — most concretely, a cryptographically
relevant quantum computer (CRQC) — breaks a primitive AmberKey relies on. The
serious variant is *harvest-now-decrypt-later*: an attacker copies age
ciphertext today and decrypts it years later once the maths is cheap. This
matters more for AmberKey than for most products, because the promise is
long-horizon confidentiality.

**What is already safe, and why.** Most of the stack is not quantum-fragile:

- **SLIP-39 secret-sharing is information-theoretically secure** — a
  below-threshold set of shares reveals *zero* information about the master
  secret, and no quantum algorithm changes that. The single most valuable
  asset — the split vault key — is post-quantum secure by construction, and it
  lives only on physical cards the server never sees.
- **ChaCha20-Poly1305 and SHA-256**: Grover's algorithm only square-roots
  symmetric strength, leaving ≥128-bit effective security at our 256-bit
  sizes — comfortable for the foreseeable future.

**The one fragile link.** Shor's algorithm breaks the **X25519 key agreement
in the age recipient stanza** (and Ed25519 on release signatures — a forgery
concern, not a confidentiality one). So a CRQC holding harvested ciphertext,
plus the vault's age *public* key, could in principle unwrap a bundle without
the cards. This is the residual quantum exposure, and it is narrow.

**Why we do not treat this as launch-blocking.**

1. The confidentiality window that actually matters is *death to estate
   settlement* — months. Most escrowed credentials (account passwords) rotate
   or die well before any CRQC arrives; the durably sensitive items (a crypto
   seed phrase) are the ones an owner should weigh here.
2. No CRQC capable of breaking X25519 exists; credible estimates put it in the
   2030s–2040s with wide uncertainty, and such a machine's first targets are
   not one consumer's estate bundle.
3. The correct fix is a **hybrid** recipient (X25519 **+** ML-KEM/FIPS 203), so
   a flaw in either component is survivable — and rolling our own or adopting
   pre-standard PQC would violate the "standard formats, no home-rolled crypto"
   rule that underwrites the exit guarantee. We wait for a reviewed hybrid
   recipient in the standard age ecosystem rather than ship a bespoke one that
   no third-party tool could recover.

**What we commit to now.** The bundle format is versioned precisely for this
(see `bundle-format.md`, "Cryptographic agility"). A future schema version can
introduce a hybrid post-quantum recipient **as an added layer**, with the
classical age v1 layer preserved so the exit guarantee — recovery with
off-the-shelf age — never breaks. Migration is a version bump, not a redesign;
already-printed cards keep working because SLIP-39 splits the *identity*, which
is orthogonal to how a given bundle is wrapped.
...
- Classical cryptanalytic breaks of X25519/ChaCha20-Poly1305/SHA-256 or the
  SLIP-39 construction; we inherit the community's confidence and would inherit
  any break alongside far larger targets. Quantum obsolescence and the
  post-quantum migration path are addressed in **T9** above, not here — it is a
  tracked, versioned concern, not an undefended gap.
